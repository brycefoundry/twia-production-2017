...
;(function ( $, window, document, undefined ) {
        ...
        var pluginName = "astero_fc",
            settings = {
                current: 'https://api.forecast.io/forecast/' + astero_fc_vars.fc_api + '/',
                geocoding: 'https://maps.googleapis.com/maps/api/geocode/json?address=',
                ip: 'http://freegeoip.net/json/',
                ip2: 'http://www.telize.com/geoip',
        ...
        function Plugin( element, options, objD ) {
                ...
                if ( (this.weather_options.lat).length == 0 || (this.weather_options.lon).length == 0 ) {
                ...
                function ip_geo_fallback(error, service) {
                        clearTimeout(location_timeout);
                        ...
                        $.ajax({
                                url: that._settings[service],
                                type: 'get',
                                crossDomain: true,
                                error: function(){
                                        if( service + '2' in that._settings ) {
                                                ip_geo_fallback('',service + '2');
                                        } else {
                                             that.error();   
                                        }
                                },
                                success: function(response){
                                        if ( typeof response.latitude != 'undefined' && typeof response.longitude != 'undefined' &&
                                                ...
                                                if ( typeof response.city != 'undefined' && response.city.length > 0 &&
                                                        typeof response.country_code != 'undefined' && response.country_code.length > 0) {
                                                           var region = typeof response.region != 'undefined' && response.region.length > 0 ? "," + response.region : '';
                                                           that.weather_options.city = response.city + region + "," + response.country_code;
                                                }
                                                
                                                that.weather_options.lat = response.latitude;
                                                that.weather_options.lon = response.longitude;
                                        } else {
                                                if( service + '2' in that._settings ) {
                                                        ip_geo_fallback('',service + '2');
                                                } else {
                        ...
                        if ( loc.val().length > 0 ) {
                                ...
                                if (geocoder) {
                                        geocoder.geocode({ 'address': loc.val() }, function (results, status) {
                                                if (status == "OK" && typeof( results[0].geometry.location.D ) != 'undefined' &&
                                                    typeof( results[0].geometry.location.k ) != 'undefined' ) {
  
                                                        if( results[0].formatted_address.length > 0) {
                                                                that.weather_options.city = results[0].formatted_address;
                                                        }
                                                        that.weather_options.lat = results[0].geometry.location.k;
                                                        that.weather_options.lon = results[0].geometry.location.D;
        ...
        Plugin.prototype = {
                init: function(that, call) {
                        ...
                        if( value ) {
                                ...
                                $.ajax({
                                        ...
                                        type: 'GET',
                                        dataType: 'jsonp',
                                        jsonpCallback : 'completeCallback',
                                        cache: true,
                                        success: function ( response ) {
                ...
                success: function(forecast, callback, call) {
                        ...
                        var that = this,
                            ...
                            icon = forecast.daily.data[0].hasOwnProperty('icon') ? forecast.daily.data[0].icon : 'clear-day',
                            condition = forecast.daily.data[0].hasOwnProperty('summary') ? forecast.daily.data[0].summary : '',
                            ...
                            humidity = forecast.daily.data[0].hasOwnProperty('humidity') ? forecast.daily.data[0].humidity * 100 : '',
                            wind = forecast.daily.data[0].hasOwnProperty('windSpeed') ? this._getWind(forecast.daily.data[0].windBearing) + ' ' + Math.round((forecast.daily.data[0].windSpeed + 0.00001) * 100) / 100 : 'clear-day',
                            cloudiness = forecast.daily.data[0].hasOwnProperty('cloudCover') ? forecast.daily.data[0].cloudCover * 100 : '0',
