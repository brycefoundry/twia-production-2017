<div id="astero<?php echo $id; ?>" class="astero<?php echo $service . $radius . $img; ?>" data-<?php echo $plugin_name; ?>='<?php echo $weather; ?>'>
        ...
        $input = '<input type="text" name="location" placeholder="' . __('a city or zip code', ASTERO_SLUG) . '" />';

        $select = '<select name="units"><option value="metric">' . __('&deg;C', ASTERO_SLUG) . '</option><option selected="selected" value="imperial">' . __('&deg;F', ASTERO_SLUG) . '</option></select>';
