...
class DpSocialTimeline {
	...
	function addScripts( $print = false ) 
	{
		...
		jQuery(document).ready(function() {
			...
			if(is_numeric($this->id_timeline) || isset($this->customData)) {
					...
					foreach($timeline_feeds as $key) {
						if($key->name == 'twitter') { 
							...
							foreach($twitter_arr as $twitter) {
								...
								$tw_string .= str_replace(array("http://", "https://"), "//", dpSocialTimeline_plugin_url('lib/user_timeline.php?screen_name='.trim($twitter).'&include_rts='.($key->include_retweets ? 'true' : 'false')));
							}
							$key->data = $tw_string; 
						}
						if($key->name == 'twitter_hash') { 
							...
							foreach($twitter_arr as $twitter) {
								if(trim($twitter) == "") { continue; }
								if($tw_string != "") { $tw_string .= ","; }
								$tw_string .= str_replace(array("http://", "https://"), "//", dpSocialTimeline_plugin_url('lib/search.php?q=%23'.urlencode(trim($twitter))));
							}
							$key->data = $tw_string; 
						}
						if($key->name == 'twitter_search') { 
							$tw_string = "";
							$twitter_arr = explode(",", $key->data);
							foreach($twitter_arr as $twitter) {
								if(trim($twitter) == "") { continue; }
								if($tw_string != "") { $tw_string .= ","; }
								$tw_string .= str_replace(array("http://", "https://"), "//", dpSocialTimeline_plugin_url('lib/search.php?q='.urlencode(trim($twitter))));
							}
							$key->data = $tw_string; 
						}
						if($key->name == 'facebook_page') { 
							...
							foreach($fb_arr as $fb) {
								...
								$fb_string .= str_replace(array("http://", "https://"), "//", dpSocialTimeline_plugin_url('lib/facebook_page.php?page_id='.trim($fb)));
							}
							$key->data = $fb_string; 
						}
						if($key->name == 'youtube') { 
							$yt_string = "";
							$yt_arr = explode(",", $key->data);
							foreach($yt_arr as $yt) {
								if(trim($yt) == "") { continue; }
								if($yt_string != "") { $yt_string .= ","; }
								$yt_string .= str_replace(array("http://", "https://"), "//", dpSocialTimeline_plugin_url('lib/youtube.php?username='.trim($yt)));
							}
							$key->data = $yt_string; 
						}
						if($key->name == 'youtube_search') { 
							$yt_search_string = "";
							$yt_search_arr = explode(",", $key->data);
							foreach($yt_search_arr as $yt_search) {
								if(trim($yt_search) == "") { continue; }
								if($yt_search_string != "") { $yt_search_string .= ","; }
								$yt_search_string .= str_replace(array("http://", "https://"), "//", dpSocialTimeline_plugin_url('lib/youtube.php?q='.trim($yt_search)));
							}
							$key->data = $yt_search_string; 
						}
						if($key->name == 'foursquare') { 
							...
							foreach($foursquare_arr as $foursquare) {
								...
								$fq_string .= str_replace(array("http://", "https://"), "//", dpSocialTimeline_plugin_url('lib/foursquare.php?url='.urlencode(trim($foursquare))));
							}
							$key->data = $fq_string; 
						}
						if($key->name == 'instagram') { 
							...
							foreach($instagram_arr as $instagram) {
								...
								$instagram_string .= str_replace(array("http://", "https://"), "//", dpSocialTimeline_plugin_url('instagram_auth/instagram.php?username='.trim($instagram)));
							}
						...
						if($key->name == 'instagram_hash') { 
							...
							foreach($instagram_hash_arr as $instagram_hash) {
								...
								$instagram_hash_string .= str_replace(array("http://", "https://"), "//", dpSocialTimeline_plugin_url('instagram_auth/instagram_hash.php?tag='.trim($instagram_hash)));
							}
				...
				';
				if(is_numeric($this->timeline_obj->cacheTime) && is_numeric($this->timeline_obj->cache)) {
				//die("data: ".$html);
					if($this->timeline_obj->cache_data == "" || (time() - $this->timeline_obj->cacheTime) > $this->timeline_obj->cache_last_update) {
						$script .= 'updateTimeline: true,';
					} 
				}
				$script .= '
	...
	function saveCache($data = "") {
		...
		if(!is_numeric($this->id_timeline)) { return false; }
		
		$wpdb->update( 
			$this->table, 
			array( 
				'cache_data' => $data,	// string
			), 
			array( 'ID' => $this->id_timeline ), 
			array( 
				'%s',	// value1
			), 
			array( '%d' ) 
