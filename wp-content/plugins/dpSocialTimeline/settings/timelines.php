...
function dpSocialTimeline_timelines_page() {
	...
	if (isset($_POST['submit']) && $_POST['submit']) {
	   ...
	   if(!isset($addColorbox) || $addColorbox != 1) { $addColorbox = 0; }
	   if(!isset($showSocialIcons) || $showSocialIcons != 1) { $showSocialIcons = 0; }
	   if(!isset($cache) || $cache != 1) { $cache = 0; }
	   if(!isset($allowMultipleFilters) || $allowMultipleFilters != 1) { $allowMultipleFilters = 0; }
	   if(!isset($showFilter) || $showFilter != 1) { $showFilter = 0; }
	   if(!isset($showLayout) || $showLayout != 1) { $showLayout = 0; }
	   if(!isset($showTime) || $showTime != 1) { $showTime = 0; }
	   if(!isset($share) || $share != 1) { $share = 0; }
	   if(!isset($rtl) || $rtl != 1) { $rtl = 0; }
	...
	if (isset($_GET['delete_timeline']) && $_GET['delete_timeline']) {
        ...
        // <![CDATA[
            function changeSocialFeed(inp) {
                ...
                switch(jQuery(inp).val()) {
					...
					case 'youtube':
                        title_desc = 'Username';
						username_desc = 'Introduce the youtube username, and don\'t forget to set the <a href="<?php echo dpSocialTimeline_admin_url( array( 'page' => 'dpSocialTimeline-twitter' ) )?>" target="_blank">API key</a>';
                        break;
					case 'youtube_search':
                        title_desc = 'Search';
						username_desc = 'Introduce the keyword, and don\'t forget to set the <a href="<?php echo dpSocialTimeline_admin_url( array( 'page' => 'dpSocialTimeline-twitter' ) )?>" target="_blank">API key</a>';
                        break;
					case 'foursquare':	
	...
	<div style="clear:both;"></div>
 	...
 	<div id="dp_ui_content">
		...
		<?php if((!isset($_GET['add']) || !is_numeric($_GET['add'])) && (!isset($_GET['edit']) || !is_numeric($_GET['edit']))) {	?>
        ...
        <?php } elseif((isset($_GET['add']) && is_numeric($_GET['add'])) || (isset($_GET['edit']) && is_numeric($_GET['edit']))) {
             ...
             <div class="col-left">
              ...
              <div class="slideboxes dp-sortable">
              	<div class="postbox slidebox">
                    <button type="button" class="handlediv button-link" aria-expanded="true"><span class="screen-reader-text"><?php _e('Show/Hide Section', 'dpSocialTimeline'); ?></span><span class="toggle-indicator" aria-hidden="true"></span></button>
